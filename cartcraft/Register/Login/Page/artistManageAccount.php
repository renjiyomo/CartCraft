...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_sql = "UPDATE artists SET names = ?, email = ?, password = ?, phone_number = ?, municipality = ?, province = ?, image = ? WHERE artist_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    
    $update_stmt->bind_param("sssssssi", $names, $email, $password, $phone_number, $municipality, $province, $profileImage, $user_id);
    
    if ($update_stmt->execute()) {
        echo "<script>alert('Profile updated successfully!');</script>";
        header("Location: artistManageAccount.php");
        exit();
    } else {
