...
$userId = $_SESSION['artist_id'] ?? null;
...
$sql = "SELECT * FROM products WHERE product_type = '$productType' AND product_status = '$productStatus' AND artist_id = '$userId'";
...
if (isset($_GET['query'])) {
    ...
    $sql = "
        SELECT product_id, product_name AS suggestion, product_image FROM products WHERE product_name LIKE '%$query%' AND artist_id = '$userId' 
        UNION 
        SELECT product_id, medium AS suggestion, product_image FROM products WHERE medium LIKE '%$query%' AND artist_id = '$userId'
    ";
...
<div class="fixed-price-text">
    ...
    <a href="addprod.php" >
        <button class="add-prod">Add Products</button>
    </a>
    ...
    <?php while ($row = mysqli_fetch_assoc($result)): ?>
        <div class="product-container">
            <div class="image-frame">
                <a href="artistProductDetail.php?id=<?php echo $row['product_id']; ?>">
...
<script>
        ...
        suggestionItems.forEach(item => {
            item.addEventListener('click', function() {
                ...
                window.location.href = 'artistProductDetail.php?id=' + productId;
            });
        });
    </script>
</body>
